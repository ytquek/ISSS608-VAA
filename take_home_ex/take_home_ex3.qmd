...
Following that, the centrality threshold will need to be defined so that most false positives are being filtered out through it.
...
We first will need to sift out edges with some form of ownership or working relationship.
Following that, the centrality threshold will need to be defined so that most false positives are being filtered out through it and then finally using *slice* to only take the top 50 edges.
