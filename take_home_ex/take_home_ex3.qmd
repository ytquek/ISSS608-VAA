...
# Convert id column to integer to match the type with from and to columns
atypical_nodes <- atypical_nodes %>%
...
p <- ggraph(graph_layout) +
  ...
  geom_node_point(aes(size = betweenness_centrality, color = closeness_centrality, 
                      tooltip = paste("ID:", id, "<br>Betweenness:", betweenness_centrality, 
                                      "<br>Closeness:", closeness_centrality))) + # Customize node appearance
  geom_node_text(aes(x = x, y = y, label = id), repel = TRUE, size = 3, check_overlap = TRUE, 
                 data = graph_layout %>% as_tibble() %>% filter(id %in% top_nodes$id)) + # Add labels to top nodes
  scale_color_viridis_c() + # Use viridis color scale
  theme_void() + # Use a void theme
  labs(title = "Top 50 Links in Atypical Business Transactions",
       subtitle = "Nodes colored by closeness centrality and sized by betweenness centrality",
